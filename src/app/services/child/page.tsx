...
const developmentalStages = [
  {
    ...
    description:
      "발달 특성에 맞춘 미술 매체가 중요한 시기입니다. 점토·핑거페인트·간단한 스토리 드로잉처럼 감각 기반 매체를 중심으로, 아동이 이야기를 만들어 낼 수 있는 안전한 장을 먼저 만듭니다.",
  },
  {
    range: "만 8 – 12세 (학령기)",
    approach: "서사와 미술의 통합",
...
const whereItHelps = [
  ...
  },
  {
    ...
    description:
      "부모의 이혼, 조부모 사별, 전학 등 큰 변화 앞에서 아동이 느끼는 모호한 감정을 안전하게 풀어냅니다. 서사를 다시 쓰는 작업이 회복을 돕습니다.",
  },
  {
    title: "정서 조절 곤란 · 충동성",
    description:
      "진단과 무관하게, 주의·충동 조절이 힘든 아동에게는 구조화된 매체(반복 패턴, 스탬프, 점토)가 신경계 안정에 도움이 됩니다.",
  },
  {
    title: "불안 · 트라우마 경험",
    description:
...
const sessionFlow = [
  ...
  },
  {
    step: "3",
    title: "나눔과 정리 (5~10분)",
    description: "완성 여부와 무관하게, 아동이 원하는 만큼만 말로 옮깁니다. 자연스럽게 작업을 정리하며 마무리합니다.",
  },
  {
    step: "4",
    title: "부모 상담 (10분)",
    description: "오늘 회기의 큰 흐름, 가정에서의 지원 방식, 다음 회기 전까지의 관찰 포인트를 나눕니다. 구체적인 작품 내용은 아동의 동의 범위 안에서 공유합니다.",
  },
];
...
const parentFaq = [
  {
    q: "변화가 언제쯤 보이나요?",
    a: "아동 미술치료는 보통 6~8회기부터 가정이나 학교에서 작은 변화(표현이 늘어남, 분노 조절이 조금 수월해짐 등)가 관찰되기 시작합니다. 단, 아동마다 속도가 매우 다릅니다. '더 이상 악화되지 않는 것'도 중요한 변화로 읽습니다.",
  },
  {
    q: "아동이 처음엔 싫다고 하면 어쩌죠?",
    a: "첫 회기에 바로 작업을 강요하지 않습니다. 세션 공간을 둘러보는 것부터 시작해, 아동의 속도에 맞추어 관계를 쌓습니다. 2~3회기 동안 '오기 싫지 않은 공간'으로 자리잡는 것이 목표입니다.",
  },
  {
    q: "미술에 자신이 없는 아동도 괜찮나요?",
  ...
  },
  {
    ...
    a: "개인 미술치료는 개별 아동에 최적화되어 있기 때문에 동시에 진행하지 않습니다. 형제 주제가 핵심일 경우, 부모 상담을 병행하며 순차적으로 구조를 설계합니다. 케이스에 따라 짝 치료도 고려 할 수 있습니다.",
  },
];
...
const confidentiality = [
  ...
  },
  {
    title: "부모 상담의 역할",
    description:
      "부모 상담은 '아동의 뒷이야기'를 듣는 시간이 아니라, 가정에서의 지원 방식을 함께 설계하는 시간입니다. 부모 자신의 정서 소진을 다룰 필요가 있는 경우 별도 세션을 권해 드립니다.",
  },
];
      ...
      <SectionWrapper bg="paper">
        <Container>
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-charcoal/70 text-sm leading-relaxed">
              아동이 최근 몇 달간 보여 온 변화 중 몇 가지가 아래 리스트와 겹친다면,
      ...
      <SectionWrapper bg="cream">
        <Container>
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-charcoal/70 text-sm leading-relaxed">
              아동이 무엇을 경험하게 되는지 부모님이 미리 상상하실 수 있도록
      ...
      <SectionWrapper bg="paper">
        <Container>
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-charcoal/70 text-sm leading-relaxed">
              상담 문의 중 가장 자주 듣는 질문들에 먼저 답을 드립니다. 아동마다
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            ...
            >
              ...
              <p className="mt-2 text-charcoal/70 text-sm leading-relaxed">
                아동을 돌보는 일이 부모님의 정서 소진으로 이어진다면, 부모님 자신의 회복을 위한 1:1 세션을 권해 드립니다.
              </p>
            </Link>
            <Link
              href="/services/group"
        ...
        <Container>
          <div className="text-center">
            ...
            <p className="mt-4 text-white/80 max-w-xl mx-auto text-sm leading-relaxed">
              첫 부모 상담은 무료로 진행됩니다. 아동의 상태와 기대하시는 변화
